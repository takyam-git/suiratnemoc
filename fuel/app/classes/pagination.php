...
class Pagination extends Fuel\Core\Pagination {
	 ...
	 */
	public static $template = array('wrapper_start' => '<div class="pagination"><ul> ', 'wrapper_end' => ' </ul></div>', 'page_start' => '<li>', 'page_end' => '</li>', 'previous_start' => '<li class="prev">', 'active_previous_start' => '<li class="prev disabled">', 'previous_end' => '</li>', 'previous_mark' => '&laquo; ', 'next_start' => '<li class="next"> ', 'active_next_start' => '<li class="next disabled">', 'next_end' => ' </li>', 'next_mark' => ' &raquo;', 'active_start' => '<li class="active"><a>', 'active_end' => '</a></li>', );
	...
	public static function page_links() {
		...
		for ($i = $start; $i <= $end; $i++) {
			if (static::$current_page == $i) {
				...
				$url = ($i == 1) ? '' : '/' . $i;
				$pagination .= static::$template['page_start'] . \Html::anchor(rtrim(static::$pagination_url, '/') . $url . static::$pagination_param, $i) . static::$template['page_end'];
			}
		}
		return $pagination;
	}
	...
	public static function next_link($value) {
		...
		if (static::$current_page == static::$total_pages) {
			return static::$template['active_next_start'] . "<a>" . $value . static::$template['next_mark'] . "</a>" . static::$template['next_end'];
		} else {
			$next_page = static::$current_page + 1;
			return static::$template['next_start'] . \Html::anchor(rtrim(static::$pagination_url, '/') . '/' . $next_page . static::$pagination_param, $value . static::$template['next_mark']) . static::$template['next_end'];
		}
	}
	...
	public static function prev_link($value) {
		...
		if (static::$current_page == 1) {
			return static::$template['active_previous_start'] . "<a>" . static::$template['previous_mark'] . $value . "</a>" . static::$template['previous_end'];
		} else {
			$previous_page = static::$current_page - 1;
			$previous_page = ($previous_page == 1) ? '' : '/' . $previous_page;
			return static::$template['previous_start'] . \Html::anchor(rtrim(static::$pagination_url, '/') . $previous_page . static::$pagination_param, static::$template['previous_mark'] . $value) . static::$template['previous_end'];
		}
	}
